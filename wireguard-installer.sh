...
# Fork of other installer by hideri.co
RED='\033[0;31m'
ORANGE='\033[0;33m'
NC='\033[0m'
...
function checkOS() {
	...
	if [[ -e /etc/debian_version ]]; then
		...
		echo "Looks like you aren't running this installer on a Debian, Ubuntu, Fedora, Oracle, Rocky or Arch Linux system"
		exit 1
	fi
}
...
function uninstallWg() {
	...
	if [[ $REMOVE == 'y' ]]; then
		...
		if [[ ${OS} == 'ubuntu' ]]; then
			apt autoremove --purge -y wireguard qrencode
		elif [[ ${OS} == 'debian' ]]; then
			apt autoremove --purge -y wireguard qrencode
		elif [[ ${OS} == 'fedora' ]]; then
			dnf autoremove -y wireguard-tools qrencode
			if [[ ${VERSION_ID} -lt 32 ]]; then
